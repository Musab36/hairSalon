...
public class App {
  public static void main(String[] args) {
    ...
    post("/clients", (request, response) -> {
          Map<String, Object> model = new HashMap<String, Object>();
          Client client = Client.find(Integer.parseInt(request.queryParams("stylistId")));
          String name = request.queryParams("name");
          Client newClient = new Client(name, client.getStylistId());
          newClient.save();
        ...
        post("/clients", (request, response) -> {
          Map<String, Object> model = new HashMap<String, Object>();
          ArrayList<Client> client = request.session().attribute("client");
          if (client == null) {
            client = new ArrayList<Client>();
            request.session().attribute("client", client);
          }
          Client myClient = Client.find(Integer.parseInt(request.queryParams("stylistId")));
          String name = request.queryParams("name");
          Client newClient = new Client(name, myClient.getStylistId());
          newClient.save();
          model.put("template", "templates/stylist-client-success.vtl");
          return new ModelAndView(model, layout);
        }, new VelocityTemplateEngine());
